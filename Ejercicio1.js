...
const textoLimpio = texto.replace(/[^a-zA-Z\s]/g, '');

const palabras = textoLimpio.split(' ');

const palabrasPorTamano = {};

for (let i = 0; i < palabras.length; i++) {
  const palabra = palabras[i];
  const tamano = palabra.length;

  if (!palabrasPorTamano[tamano]) {
    palabrasPorTamano[tamano] = [];
  }

  palabrasPorTamano[tamano].push(palabra);
}

const resultado = Object.values(palabrasPorTamano);
console.log(palabrasPorTamano);
